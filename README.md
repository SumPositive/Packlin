...
- 移動・複製 — アイテムはグループ間・パック間で移動または複製できます。グループもパック間で移動または複製できます。アイテム一覧からは「まとめて移動」も可能です。
...
- パック公開 — パックをクラウドに公開でき、他の利用者が公開したパックを取り込めます。共有リンクはインストール済みのPacklinで開くと起動時に自動取込します。公開中のパックは一覧にバッジ表示します。
- iCloud バックアップ — AI 利用券とパック公開用の ID を iCloud に保存し、端末交換時に復元します。
...
- 外観モード — 自動、ライト、ダーク
- 明細表示 — 極小、最小、1行、2行、3行
- 文字サイズ — 自動（システム設定に追従）、標準、大、特大（ローガンフリー）
- 新規追加位置 — 先頭、末尾
...
チャッピー、チケット、パック公開などの一部機能では、軽量なバックエンド `azuki-api` を利用します。

- 認証 — ログインや個人情報登録なしの端末ベース認証
- 購入 — アプリ内課金チケットの検証
- 広告 — リワード広告視聴によるチケット付与
- パック公開 — パックのクラウド公開・一覧・検索・取り込み・公開取消

端末を識別する `userId` と AI 利用券残高は iCloud Key-Value Store にバックアップし、端末交換や故障時に復元できるようにしています。
...
- Associated Domainsは`applinks:azuki-api.azukid.com`
- 対象パスは`/packlin/public/*`
- Team IDは`5C2UYK6F45`、Bundle IDは`com.azukid.AzPackList5`
- APIを先にデプロイし、AASAがHTTPSでリダイレクトなしに取得できる状態で実機へインストールする
- 共有リンクをタップすると公開パックを自動取込し、通常のパック一覧へ表示する
