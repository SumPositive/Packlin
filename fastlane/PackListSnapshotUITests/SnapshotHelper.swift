...
// Swift 6 の並行性チェック対策。UITest は単一プロセス・逐次実行なので unsafe で問題ない。
...
open class Snapshot: NSObject {
    ...
    class func setLocale(_ app: XCUIApplication) {
        ...
        if !locale.isEmpty {
            // 注意: 値をクォートで囲むと Locale.current に反映されず通貨が既定のままになる。
            // クォートなしで渡すこと。
...
private extension XCUIElementQuery {
    var networkLoadingIndicators: XCUIElementQuery {
        let isNetworkLoadingIndicator = NSPredicate(format: "identifier CONTAINS[c] 'network' AND identifier CONTAINS[c] 'loading'")
    ...
    var deviceStatusBars: XCUIElementQuery {
        ...
        let isStatusBar = NSPredicate { (evaluatedObject, _) in
            ...
            return element.frame.origin.y == 0.0 && element.frame.width == deviceWidth
        }
